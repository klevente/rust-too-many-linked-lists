// stack-allocated `List`, which is managed by callbacks to allocate more stack!
pub struct List<'a, T> {
    pub data: T,
...
impl<'a, T> List<'a, T> {
    pub fn push<U>(
        // take the already existing `List`
        prev: Option<&'a List<'a, T>>,
        // and a new element
        data: T,
        // and a function, in which we have the new `List` with the new element!
...
mod test {
    ...
    #[test]
    fn elegance() {
        // use callbacks to access the modified `List`!
    ...
    fn cell() {
        ...
        // mutability can be achieved with interior mutability, e.g. `Cell`!
